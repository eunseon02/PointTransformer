...
class Point(Dict):
    ...
    def serialization(self, order="z", depth=None, shuffle_orders=False):
        ...
        if "grid_coord" not in self.keys():
            ...
            coord_no_nan = self["coord"].masked_fill(torch.isnan(self["coord"]), float('inf'))
            self["grid_coord"] = torch.div(
                self.coord - coord_no_nan.min(0)[0], self.grid_size, rounding_mode="trunc"
            ).int()
            # print("grid_coord", self["grid_coord"])
            # self["grid_coord"] = torch.div(
            #     self.coord - torch.tensor([-8, -8, -3]), self.grid_size, rounding_mode="trunc"
            # ).int()


        if depth is None:
            # Adaptive measure the depth of serialization cube (length = 2 ^ depth)
            # print(self.grid_coord[:, :5])
